...
```
This configuration file sets a 'requestCountThreshold' of 5 to enable throttling,
returning the HTTP response code 429 to the client when the total request count reaches 5.
It's important to note that this configuration is solely intended for demonstration purposes
and should not be utilized in any situation other than for testing purposes.

If the 'EndpointMetricsThrottlingManager' doesn't meet your requirements, you have the option to
supply your own ThrottlingManager. Below is a straightforward example demonstrating
how to throttle traffic after three faults occur:
```
...
```
To enable this TestThrottlingManager, the jbossws-endpoint-config.xml should be provided
